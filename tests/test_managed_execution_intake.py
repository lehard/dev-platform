...
class ManagedExecutionIntakeTests(unittest.TestCase):
    ...
    def test_fresh_execution_composes_authoring_then_exact_managed_start(self) -> None:
        ...
        with patch.object(execute_managed_task, "admit_managed_intake") as admission, patch.object(
            execute_managed_task, "create_task", return_value=(package, False, False)
        ) as create, patch.object(execute_managed_task, "start_managed_task", return_value=(started, "a" * 40, False)) as start:
        ...
        admission.assert_called_once_with(Path("/root"))
    ...
    def test_exact_existing_issue_is_reused_instead_of_creating_a_duplicate(self) -> None:
        ...
        with patch.object(execute_managed_task, "admit_managed_intake"), patch.object(
            execute_managed_task, "create_task", side_effect=duplicate
        ), patch.object(execute_managed_task, "start_managed_task", return_value=(started, "a" * 40, True)) as start:
            result = execute_managed_task.execute_managed_task(Path("/root"), self.args())
        self.assertEqual(result[:3], ("owner/backlog#42", True, True))
        self.assertEqual(start.call_args.args[1], "owner/backlog#42")

    def test_admission_failure_prevents_composed_authoring_mutation(self) -> None:
        with patch.object(execute_managed_task, "admit_managed_intake", side_effect=RuntimeError("session blocked")), patch.object(
            execute_managed_task, "create_task"
        ) as create:
            with self.assertRaisesRegex(RuntimeError, "session blocked"):
                execute_managed_task.execute_managed_task(Path("/root"), self.args())
