...
class ManagedProjectPromotionTests(unittest.TestCase):
    def test_candidate_promotes_in_place(self) -> None:
        ...
        changed = managed_projects.promote_repository(data, "example-org/example", "main", "adopted")
    ...
    def test_explicit_adoption_can_reclassify_excluded_placeholder(self) -> None:
        ...
        changed = managed_projects.promote_repository(data, "example-org/example", "main", "activated")
        self.assertTrue(changed)
        self.assertEqual(data["projects"][0]["state"], "managed")

    def test_unknown_repository_is_appended(self) -> None:
        data = {"schema_version": 1, "projects": []}
        managed_projects.promote_repository(data, "example-org/new-project", "trunk", "adopted")
        self.assertEqual(data["projects"][0]["repository"], "example-org/new-project")
    ...
    def test_promotion_is_idempotent(self) -> None:
        ...
        changed = managed_projects.promote_repository(data, "example-org/example", "main", "same")
