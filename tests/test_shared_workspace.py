...
class SharedWorkspaceTests(unittest.TestCase):
    ...
    def test_atomic_replacement_repairs_hostile_creation_mask(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "state.json"
            original = os.umask(0o077)
            try:
                shared_workspace.atomic_write_text(path, '{"safe": true}\n')
            finally:
                os.umask(original)
    ...
    def test_managed_intake_probe_cleans_only_its_own_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            # The host temporary volume used by tests does not retain setgid,
            # so isolate probe behavior from that filesystem quirk.
            with mock.patch.object(shared_workspace, "_describe", return_value=None):
    ...
    def test_managed_intake_probe_rejects_restrictive_session_umask_before_writing(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "project"
            root.mkdir()
            self.init_repo(root)
            ...
            try:
                with self.assertRaisesRegex(shared_workspace.SharedWorkspaceError, "umask is 0022"):
                    shared_workspace.session_admission_probe(root)
            finally:
                os.umask(original)
            self.assertEqual(list(root.glob(".dev-platform-managed-intake-*")), [])

    def test_admission_probe_self_chmods_a_freshly_created_directory_that_did_not_inherit_setgid(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "project"
            root.mkdir()
            self.init_repo(root)
            ...
            try:
                # No ``_describe`` mock here: this proves the probe can
                # establish the platform contract on its own freshly created
                # artifacts even when the OS does not auto-inherit the
                # parent's setgid bit onto a newly ``mkdir``'d child (observed
                # on this project's macOS/APFS test hosts -- see the sibling
                # test above). A filesystem that *does* inherit setgid passes
                # the same assertion trivially, since the self-repair step is
                # then a no-op.
                shared_workspace.session_admission_probe(root)
            finally:
                os.umask(original)
    ...
    def test_admission_probe_fails_closed_on_wrong_group(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "project"
            root.mkdir()
            self.init_repo(root)
            root.chmod(0o2775)
            wrong_group = shared_workspace.SharedGroup(gid=os.getegid() + 999999, name="not-mine", source="test")
            with mock.patch.object(shared_workspace, "resolve_shared_group", return_value=wrong_group):
                with self.assertRaisesRegex(shared_workspace.SharedWorkspaceError, "not effectively a member"):
                    shared_workspace.session_admission_probe(root)
            self.assertEqual(list(root.glob(".dev-platform-managed-intake-*")), [])

    def test_admission_probe_fails_closed_when_it_cannot_chmod_its_own_probe_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "project"
            root.mkdir()
            self.init_repo(root)
            ...
            with mock.patch.object(
                shared_workspace, "_repair", side_effect=shared_workspace.SharedWorkspaceError("unrepairable shared path")
            ):
                with self.assertRaises(shared_workspace.SharedWorkspaceError):
                    shared_workspace.session_admission_probe(root)
            self.assertEqual(list(root.glob(".dev-platform-managed-intake-*")), [])

    def test_admission_probe_fails_closed_when_group_write_cannot_be_verified_after_repair(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "project"
            root.mkdir()
            self.init_repo(root)
            root.chmod(0o2775)
            # ``_repair`` runs (and does not raise) but the directory still
            # fails its post-repair contract check -- for example a
            # filesystem that silently refuses the setgid bit entirely.
            with mock.patch.object(shared_workspace, "_describe", return_value="mode=0775, missing group rwx+setgid"):
                with self.assertRaisesRegex(shared_workspace.SharedWorkspaceError, "group-writable setgid probe directory"):
                    shared_workspace.session_admission_probe(root)
            self.assertEqual(list(root.glob(".dev-platform-managed-intake-*")), [])

    def test_admission_probe_fails_closed_when_atomic_replace_cannot_be_verified(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "project"
            root.mkdir()
            self.init_repo(root)
    ...
    def test_admission_aggregates_session_and_every_historical_finding(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "project"
            root.mkdir()
            first = root / "first-state.json"
            second = root / "second-state.json"
            first.write_text("{}\n", encoding="utf-8")
            second.write_text("{}\n", encoding="utf-8")
            group = shared_workspace.resolve_shared_group(root)
            findings = [
                shared_workspace.Finding(first, "mode=0600, missing group rw", True),
                shared_workspace.Finding(second, "gid=999, expected group staff", True),
            ]
            with (
                mock.patch.object(shared_workspace, "shared_workspace_applicable", return_value=True),
                mock.patch.object(shared_workspace, "session_admission_probe", side_effect=shared_workspace.SharedWorkspaceError("launcher lacks group")),
                mock.patch.object(shared_workspace, "audit", return_value=(group, findings)),
            ):
                with self.assertRaises(shared_workspace.SharedWorkspaceError) as raised:
                    shared_workspace.admit_managed_intake(root)
            message = str(raised.exception)
            self.assertIn("live session: launcher lacks group", message)
            self.assertIn(str(first), message)
            self.assertIn(str(second), message)
            self.assertIn("owner", message)
            self.assertIn("chgrp", message)

    def test_github_hosted_disposable_context_is_not_admission_gated(self) -> None:
        root = Path("/definitely/not/a/checkout")
        with mock.patch.dict(os.environ, {"GITHUB_ACTIONS": "true", "RUNNER_ENVIRONMENT": "github-hosted"}, clear=False):
            self.assertFalse(shared_workspace.shared_workspace_applicable(root))
