...
class WorktreeCleanupTests(WorktreeHarnessCase):
    ...
    def test_squash_merged_exact_head_worktree_becomes_eligible(self) -> None:
        path = self.add_worktree("squash-merged")
        # No merge into main at all: this models a squash merge, where the
        # rewritten commit on main is never an ancestor of the original
        # branch tip, exactly like a real not-yet-merged branch as far as
        # ancestry is concerned.
        worktree = self.listed(path)
        activity = worktree_cleanup._activity_timestamp(self.root, worktree)
        merged = publication_state.ExactHeadPrLookup(
            available=True, exact_merged={"number": 438, "state": "MERGED", "headRefOid": worktree.head}
        )
        with (
            mock.patch.object(worktree_cleanup, "github_cli_env", return_value={}),
            mock.patch.object(worktree_cleanup.publication_state, "find_exact_head_pr", return_value=merged) as lookup,
        ):
            decision = worktree_cleanup.classify(
                self.root,
                worktree,
    ...
    def test_open_pr_worktree_stays_ineligible(self) -> None:
        ...
        worktree = self.listed(path)
        open_pr = publication_state.ExactHeadPrLookup(available=True, exact_open={"number": 9, "state": "OPEN"})
        with (
            mock.patch.object(worktree_cleanup, "github_cli_env", return_value={}),
            mock.patch.object(worktree_cleanup.publication_state, "find_exact_head_pr", return_value=open_pr),
        ):
            decision = worktree_cleanup.classify(
                self.root,
                worktree,
        ...
        self.assertEqual(decision.reason, "not-merged")

    def test_mismatched_pr_head_worktree_stays_ineligible(self) -> None:
        path = self.add_worktree("mismatched-head")
        worktree = self.listed(path)
        # Available and structurally valid, but neither exact_open nor
        # exact_merged matched this worktree's exact head.
        mismatched = publication_state.ExactHeadPrLookup(available=True)
        with (
            mock.patch.object(worktree_cleanup, "github_cli_env", return_value={}),
            mock.patch.object(worktree_cleanup.publication_state, "find_exact_head_pr", return_value=mismatched),
        ):
            decision = worktree_cleanup.classify(
                self.root,
                worktree,
    ...
    def test_unavailable_github_evidence_worktree_stays_ineligible(self) -> None:
        path = self.add_worktree("gh-unavailable")
        worktree = self.listed(path)
        unavailable = publication_state.ExactHeadPrLookup(available=False, detail="GitHub CLI/API authentication is unavailable")
        with (
            mock.patch.object(worktree_cleanup, "github_cli_env", return_value={}),
            mock.patch.object(worktree_cleanup.publication_state, "find_exact_head_pr", return_value=unavailable),
        ):
            decision = worktree_cleanup.classify(
                self.root,
                worktree,
    ...
    def test_dirty_worktree_with_merged_pr_mock_stays_ineligible(self) -> None:
        dirty = self.add_worktree("dirty-but-merged")
        (dirty / "uncommitted.txt").write_text("uncommitted\n", encoding="utf-8")
        worktree = self.listed(dirty)
        merged = publication_state.ExactHeadPrLookup(
            available=True, exact_merged={"number": 1, "state": "MERGED", "headRefOid": worktree.head}
        )
        with (
            mock.patch.object(worktree_cleanup, "github_cli_env", return_value={}),
            mock.patch.object(worktree_cleanup.publication_state, "find_exact_head_pr", return_value=merged) as lookup,
        ...
        # The dirty check runs before any merge proof is attempted at all.
