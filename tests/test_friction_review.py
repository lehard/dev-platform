...
class FrictionReviewTests(unittest.TestCase):
    ...
    def test_archived_route_provenance_survives_an_unrelated_active_change(self) -> None:
        self.write_route(
            participant={"role": "executor", "provider": "claude", "model": {"value": "sonnet", "source": "selected"}}
        )
        archived = self.root / "openspec" / "changes" / "archive" / "2026-09-19-routing-change"
        archived.parent.mkdir(parents=True)
        shutil.move(str(self.root / "openspec" / "changes" / "routing-change"), str(archived))
        unrelated = self.root / "openspec" / "changes" / "unrelated"
        unrelated.mkdir()
        (unrelated / ".managed-task.json").write_text(
            json.dumps({"source_issue": "owner/backlog#10", "change": "unrelated"}), encoding="utf-8"
        )
        provenance = agent_friction._current_run_provenance("executor")
        self.assertEqual(provenance["source_issue"], "owner/backlog#9")
        self.assertEqual(provenance["change"], "routing-change")
        self.assertEqual(provenance["participant"]["model"]["value"], "sonnet")
