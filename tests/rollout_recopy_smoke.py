...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="dev-platform-recopy-smoke-") as tmp:
        ...
        if after_answers.get("_commit") != "v1.3.0":
            raise SystemExit(f"Copier did not record v1.3.0: {after_answers.get('_commit')}")
        if rollout_project.load_platform_version(project) != "1.3.0":
            raise SystemExit("platform_bootstrap did not synchronize platform_version to 1.3.0")
