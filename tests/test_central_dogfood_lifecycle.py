...
class CentralDogfoodLifecycleTests(unittest.TestCase):
    ...
    def route_payload(self, *, provider: str = "codex", profile: str = "standard", execution: dict | None = None) -> dict:
        return {
            "source_issue": "example-org/development-backlog#2",
            "change": "central-task",
            "task_worktree": str(self.root),
            "integration_root": str(self.root),
            "provider": provider,
            "profile": profile,
            "executor_model": "selected-model",
            "rationale": "bounded current-spec preflight",
            "prepared_at": "2026-09-19T00:00:00+00:00",
            "pre_snapshot": {"head": "a" * 40, "paths": {}},
            "execution": execution,
        }
    ...
    def test_routing_gate_accepts_successful_standard_route(self) -> None:
        ...
        record.write_text(
            json.dumps(
                self.route_payload(execution={"outcome": "completed", "launched": True, "returncode": 0, "violation": False})
            ),
            encoding="utf-8",
        )
    ...
    def test_routing_gate_rejects_claude_route_without_clean_postcheck(self) -> None:
        ...
        with self.assertRaisesRegex(SystemExit, "clean containment postcheck"):
            dogfood_task.require_routing_gate(self.root)

    def test_routing_gate_verifies_archived_exact_task_from_durable_state(self) -> None:
        change = self.add_managed_change()
        (self.root / ".managed-task-state.json").write_text(
            json.dumps({"source_issue": "example-org/development-backlog#2", "change": "central-task"}), encoding="utf-8"
        )
        archived = self.root / "openspec" / "changes" / "archive" / "2026-09-19-central-task"
        archived.parent.mkdir(parents=True)
        shutil.move(str(change), str(archived))
        unrelated = self.root / "openspec" / "changes" / "unrelated"
        unrelated.mkdir()
        (unrelated / ".managed-task.json").write_text(
            json.dumps({"source_issue": "example-org/development-backlog#3", "change": "unrelated"}), encoding="utf-8"
        )
        record = self.root / ".claude" / "model-routing" / "central-task.json"
        record.parent.mkdir(parents=True)
        record.write_text(
