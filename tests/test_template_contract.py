...
class TemplateContractTests(unittest.TestCase):
    ...
    def test_optional_capability_contract_is_separate_from_profiles_and_openspec(self) -> None:
        agents = (ROOT / "template" / "AGENTS.md.jinja").read_text(encoding="utf-8")
        manager = (ROOT / "template" / "scripts" / "capability_manager.py").read_text(encoding="utf-8")
        descriptor = (ROOT / "template" / "dev-platform" / "capabilities" / "repository-hygiene.toml").read_text(encoding="utf-8")
        self.assertIn("dev-platform/capabilities.toml", (ROOT / "copier.yml").read_text(encoding="utf-8"))
        self.assertIn("Optional engineering capabilities", agents)
        self.assertIn("OpenSpec-generated skills remain external", agents)
        self.assertIn("auto+explicit", descriptor)
        self.assertIn("blocked/unavailable", manager)
        self.assertIn("refusing to remove unowned provider skill", manager)

    def test_architecture_health_capability_is_mirrored_in_the_template(self) -> None:
        for relative in (
            "dev-platform/capabilities/architecture-health-review.toml",
            "dev-platform/capabilities/architecture-health-review.md",
            "dev-platform/evals/architecture-health-review-pilot.json",
    ...
    def test_web_engineering_capability_pack_is_mirrored_in_the_template(self) -> None:
        for relative in (
            "dev-platform/capabilities/react-next-best-practices.toml",
            "dev-platform/capabilities/react-next-best-practices.md",
            ...
            "dev-platform/evals/react-next-best-practices-pilot.json",
            "dev-platform/capabilities/ui-quality-review.toml",
            "dev-platform/capabilities/ui-quality-review.md",
            "dev-platform/evals/ui-quality-review-pilot.json",
            "docs/engineering/engineering-capabilities.md",
        ):
            with self.subTest(relative=relative):
                self.assertEqual(
