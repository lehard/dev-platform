...
The rendered `scripts/shared_workspace.py` derives the shared group from the integration checkout (or the machine-local `DEV_PLATFORM_SHARED_GROUP` override). It checks and repairs only the registered lifecycle allowlist under `.claude` (board, locks, worktree administration, friction and routing state) plus required Git common-directory metadata. Unknown `.claude` entries, including tool-managed symlinks and transient caches, are foreign and untouched. `check` is read-only; `fix` sets group write and setgid only within those registered roots, configures `core.sharedRepository=group`, and stops with the exact path and owner action when repair is not authorized. Unsupported non-POSIX filesystems use a diagnostic no-op path.
