...
- **Claude Code**: the native Agent tool is invoked by the supervisor's own tool call and its result reaches only the supervisor's conversation, so no platform-owned process observes a launch. After the supervisor invokes the hand-off and it returns, `record-claude-execution --agent-id "<id>"` records `execution.outcome: "claimed"`, `execution.launch_evidence: "self-reported"`, `execution.launched: null` (unknown), and the supplied id as `claimed_agent_id` -- never `launched: true`. There is no executed `execution.participant`; instead a `claimed_participant` carries the same role/provider/profile/model/effort/execution-id shape with the model's `source` recorded as `self-reported`. `finish`'s routing gate accepts this only on the verifiable clean containment postcheck, treating the claim as honestly labelled, not as launch proof.

A route that was only prepared -- containment unprovable, integration dirty, or the supervisor never actually invoked the hand-off -- has no execution participant at all. Fallback (parent retains the work) and escalation both preserve the actual path that ran, not the preferred path that was requested.
