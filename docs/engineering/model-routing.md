...
Optional usage measurements are normalized as `{value, source, status}`. `status: measured` is used only for a structured runtime event with an exact supported field; unavailable, partial, malformed, or ambiguous multi-completion usage is `{value: null, source: unknown, status: unknown}`, never zero. The locally inspected Codex CLI (`0.146.1`) documents the JSONL launch surface but not a stable usage schema, so the adapter accepts only an exact emitted `turn.completed.usage` shape for input, cached-input, output, and total tokens, and counts structured `turn.started` events as requests/turns. It does not derive fresh input from cached input, sum potentially cumulative completion events, scrape output, or infer a total. Claude Code's current native Agent hand-off exposes no authoritative token/cache/request-count or execution-duration surface, so those values remain unknown until an adapter can prove them.
...
The report scans local routing records across registered worktrees, preserves historical records without efficiency fields as missing evidence, counts completed/failed/abnormal outcomes and escalated routes already owned by routing provenance, and reads the existing active/archived OpenSpec verification receipt for verification context rather than creating a second status. It reports medians plus p95 only when a populated metric has at least five observations. Fewer than 15 launched executions are explicitly labelled `insufficient`; the initial decision-quality target is roughly 15–30 verified managed executions with compatible task-family/runtime coverage. This report informs later routing calibration or external-runtime comparisons only; it does not switch models, alter tiers, impose budgets, or cancel work.
