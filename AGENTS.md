...
- **Routing is a required gate.** Every managed task records a bounded semantic preflight through `scripts/dogfood_task.py route-codex` or `route-claude` before implementation. The user does not choose an executor, and a delegation is claimed only when its write containment is actually proven.
