...
When no local stable version tag exists, CI SHALL fall back to a bootstrap baseline commit. If neither a stable tag nor a resolvable bootstrap baseline exists in the current checkout's own history (for example a fresh-history public-distribution snapshot with no prior commits), the smoke test SHALL skip cleanly instead of failing, since there is no meaningful prior platform state to upgrade from in that checkout.
...
#### Scenario: Checkout has no historical baseline to upgrade from

- **GIVEN** the current checkout has no local stable version tag
- **AND** the bootstrap baseline commit does not exist in the checkout's own history
- **WHEN** the Copier-upgrade smoke test runs
- **THEN** it skips cleanly with a clear reason instead of failing
- **AND** it performs no Copier invocation before determining no baseline exists
...
Release automation SHALL first determine whether the current commit actually establishes publishable release intent before creating or confirming any tag or release. A commit with no Git parent (a repository root/bootstrap commit) SHALL NEVER establish release intent, regardless of what its `VERSION` file contains: a fresh-history bootstrap commit and a genuine first version bump are structurally identical by file content alone, so parentage is the fact that distinguishes them. A non-root commit SHALL establish release intent only when its `VERSION` value differs from its first parent's; an unchanged `VERSION` SHALL NOT trigger publication.
...
#### Scenario: A fresh-history root commit includes a valid VERSION

- **GIVEN** the current commit has no Git parent (for example a freshly bootstrapped public-history checkout)
- **AND** its `VERSION` file parses as valid SemVer
- **WHEN** release automation evaluates release intent
- **THEN** no tag is created
- **AND** no GitHub Release is created
- **AND** the run completes without publishing anything

#### Scenario: VERSION is unchanged from the parent commit

- **GIVEN** a non-root commit whose `VERSION` value is identical to its first parent's
- **WHEN** release automation evaluates release intent
- **THEN** no tag or release is created for that commit
...
Publishing an immutable platform release is a core Dev Platform capability available to any checkout. Managed fleet rollout is a distinct, optional operator capability that requires a private managed-projects registry and dedicated GitHub App credentials. Publishing a release SHALL NEVER by itself require or imply operator fleet configuration.

After publishing an immutable platform version, the central release workflow SHALL dispatch the managed-project rollout for that exact SemVer tag only when the repository has explicitly opted in via a repository variable (never a secret, since a secret's mere presence SHALL NOT double as a feature flag). Absent or non-affirmative opt-in SHALL leave the release published but SHALL NOT dispatch rollout, and the run SHALL explicitly report that managed rollout is disabled or not configured. Rollout SHALL also support an explicit manual retry path and SHALL reject versions that are not actually published platform releases.

Rollout dispatch from the release workflow SHALL occur only when the current run itself established publishable release intent (an actual version bump) AND the repository has opted in to managed rollout. A run that determines it has no release intent (including a fresh-history root/bootstrap commit or an unchanged `VERSION`) SHALL NOT dispatch rollout regardless of opt-in.

#### Scenario: New platform version is published

- **GIVEN** the repository has explicitly opted in to managed rollout via its repository variable
...
#### Scenario: A generic repository publishes a release without operator opt-in

- **GIVEN** the repository has not set the managed-rollout opt-in variable (the default for a generic public checkout)
- **WHEN** the release workflow successfully creates or confirms `vX.Y.Z` at the release commit
- **THEN** the release and tag are created normally
- **AND** rollout is not dispatched
- **AND** the run explicitly reports that managed rollout is disabled/not configured
...
#### Scenario: A bootstrap run does not dispatch rollout

- **GIVEN** the release workflow's own release-intent determination for the current commit is false (for example a fresh-history root commit)
- **WHEN** the release workflow runs
- **THEN** it does not dispatch the rollout workflow, regardless of the managed-rollout opt-in variable
...
### Requirement: Historical harness-transition smoke tests skip gracefully without their reproduction tag

A CI smoke test that reproduces a specific historical project-harness transition by checking out a specific historical release tag SHALL skip cleanly, rather than fail, when that tag does not exist in the current checkout (for example a fresh-history checkout that intentionally carries no historical release tags). This SHALL NOT weaken the check when the reproduction tag is present.

#### Scenario: Reproduction tag is absent from a fresh-history checkout

- **GIVEN** a checkout has no historical release tags
- **WHEN** a harness-transition reproduction smoke test looks for its specific historical tag
- **THEN** it skips with a clear reason and completes successfully
- **AND** it performs no Copier or Git mutation before determining the tag is absent

#### Scenario: Reproduction tag is present

- **GIVEN** the checkout has the specific historical release tag the smoke test reproduces
- **WHEN** the smoke test runs
- **THEN** it performs its full reproduction and verification exactly as before
...
#### Scenario: Selected managed project is rolled out from a clean checkout
...
- **THEN** rollout SHALL provide the generic operator-integration answer to Copier as render data while the downstream checkout remains clean
- **AND** Copier SHALL record the selected answer as part of its normal update result before bootstrap
