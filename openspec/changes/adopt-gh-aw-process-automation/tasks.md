...
- [x] 5.6 Observe at least one genuine scheduled `Weekly Process Backlog Review` run (not `workflow_dispatch`), verify the result is bounded/useful, and confirm deterministic CI/release remains independent if the agentic workflow fails. Evidence: run `31649072869` (`event: schedule`, `conclusion: success`, 2026-08-12T22:57:52Z-23:02:23Z, all 5 jobs succeeded), produced bounded advisory summary `lehard/dev-platform#221` (no Backlog task created, matching 5.7). Observed via a temporary, reverted schedule probe (PRs #217 then #222); deterministic CI/release were untouched by either change.
...
- [x] 6.1 At implementation preflight, verify the current supported Codex and Claude Code runtime surfaces for machine-readable parent-session model metadata, delegated execution identity and selected/effective reasoning effort. Record the proven surfaces and keep unsupported fields optional/unknown rather than designing against assumptions. Evidence: live `codex exec --json` run in an isolated scratch repo confirmed a real `thread.started`/`thread_id` event (no confirmed model/effort field exists); direct inspection of the actual Agent tool schema confirmed it has no `effort` parameter (`claude_agent()`'s prior fabricated `"effort"`/`"maxTurns"` keys removed).
- [x] 6.2 Extend the existing bounded model-routing/completion evidence so a task run can represent supervisor and actually executed child participants with provider/runtime, role, profile, model, reasoning effort when available, provenance source/status, bounded execution identifier and parent/child relationship. Do not add transcript/tool-call storage or a second run database. Implemented as `Route.supervisor` + `execution.participant` in `model_routing.py`; no new persistence surface.
- [x] 6.3 Preserve the selected/configured vs runtime-confirmed distinction for model and reasoning effort. Never infer effective effort from a global config, prompt, model self-report or unsupported runtime behaviour; retain `unknown` where confirmation is unavailable. `SOURCE_SELECTED`/`SOURCE_RUNTIME_CONFIRMED`/`SOURCE_UNKNOWN` vocabulary; every current value is `selected` or `unknown` -- nothing is claimed `runtime-confirmed` because no supported surface proves it yet.
- [x] 6.4 For Codex routing, reuse the current platform-owned route/launch evidence and record the actual child/fallback/escalation path. Add effort metadata only through a currently supported verified Codex surface, with truthful source/status. `run_codex()` parses the real `--json` stream for `thread_id`; effort stays `unknown` (no verified surface).
- [x] 6.5 For Claude routing, reuse the existing native Agent hand-off model/effort and post-call `agent_id` evidence; represent a child as executed only after the real Agent invocation is recorded. Verify whether stronger runtime-confirmed model/effort metadata is available before claiming it. `record_claude_execution()` attaches `participant` only after a real returned `agent_id`; effort verified unconfirmable (6.1) and kept `unknown`.
- [x] 6.6 Link friction events and retrospective findings to the current run and, when evidence permits, the relevant supervisor or child participant. Ambiguous findings remain run-level/participant-unknown rather than assigning blame by guess. `agent_friction.py record --participant-role supervisor|executor` reads identity back from the current routing record; default `unknown`.
- [x] 6.7 Include only bounded sanitized execution provenance in routed GitHub process evidence. Keep raw evidence, unnecessary machine-local identifiers, prompts/transcripts and other sensitive execution detail local by default. `route_body()`'s `_provenance_line()` emits only role/provider/model/source; execution ids stay local (tested).
- [x] 6.8 Keep friction deduplication based on the underlying process-problem fingerprint rather than model identity; model/runtime belongs to per-occurrence provenance so later review can compare patterns without issue proliferation. `fingerprint_for()` unchanged (scope+category only); regression test proves identical fingerprints across differing participants.
- [x] 6.9 Update generated shared Codex/Claude guidance and final completion reporting so agents use the platform-owned provenance instead of free-form self-identification and explicitly report unknown fields truthfully. `docs/engineering/model-routing.md` and the `template/` counterpart gained an "Execution provenance" section; `agent-workflow.md` (both) documents `--participant-role`.
- [x] 6.10 Add deterministic regression coverage for prepared-but-not-run delegation, actual child execution, parent fallback, escalation, unknown metadata, selected-vs-confirmed effort and participant attribution. 15 new/extended cases across `test_model_routing.py`, `test_friction_review.py`, `test_delegated_write_guard.py`; full suite 492 tests pass.
- [x] 6.11 Perform real controlled acceptance in both supported entrypoints: at least one Codex task and one Claude Code task with routed delegation must produce truthful bounded execution provenance and representative friction attribution; include at least one case where an unavailable/unconfirmed field remains `unknown` rather than being fabricated. **Claude leg done for real**: a live routine delegation via `dogfood_task.py route-claude` -> real Agent tool call (agent id `a7238c3adade4fd10`) -> `record-claude-execution` produced a genuine `participant` record (`model: haiku/selected`, `reasoning_effort: null/unknown`, real `execution_id`) with a clean containment postcheck; the delegated child's report also caught and self-corrected a real wrong-directory read, itself recorded as new friction (`lehard/dev-platform#213`). **Codex leg deliberately split out**: the path is implemented and unit-tested against realistic captured `--json` event lines (see 6.2/6.4/6.10), but a real live `codex` CLI run was never exercised -- the authenticated account was credit-limited until 2026-08-18 (confirmed live, not assumed). Per explicit owner decision, this change does not wait for that external reset; live Codex acceptance is tracked as a small dedicated follow-up, `lehard/development-backlog#33` (`verify-codex-live-execution-provenance`), which does not duplicate this change's implementation and only verifies what could not be verified here.
