...
- [x] 8. Publish implementation through a normal protected-main PR. (#93, squash-merged 409ed07)
- [x] 9. After merge, cut the next normal cumulative immutable patch release (do not retag/reuse v1.4.21) and confirm the resulting automatic managed rollout reaches a normal per-project result (opened/reused/up-to-date PR) for every `managed` repository, with no unsupported-`gh`-flag failure, no missing-helper-path failure, and no failure-streak label crash. v1.4.22 (#94) published; the auto-dispatched rollout (run 31469600804) succeeded for all three managed repos: planner-agent-lab, Jara_Fin, and cuby (opened lehard/cuby#46, merged after green downstream CI). No unsupported-gh-flag or missing-helper-path failure occurred.
- [x] 10. Record `OpenSpec-Verify: PASS` with the real method in `verification.md`, archive via `python3 template/scripts/openspec_lifecycle.py archive repair-managed-rollout-control-plane`, then publish the archive/spec result through protected main.
