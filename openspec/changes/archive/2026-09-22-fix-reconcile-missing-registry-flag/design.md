...
Both of these jobs run on their own fresh GitHub Actions runner, separate from their respective `plan` job -- job outputs carry only explicitly declared step outputs (strings), never files or checked-out working trees, so `plan`'s `operator/` checkout cannot be reused. Each job therefore needs its own equivalent "operator repository" -> "registry read token" -> "checkout operator registry" step sequence, mirroring the mechanism already used in the corresponding `plan` job, then `--registry operator/managed-projects.json` passed at each of the three `reconcile` call sites. The new sequences intentionally omit the `plan` job's own `APP_CLIENT_ID`/`APP_PRIVATE_KEY` presence check, since both the `rollout` job and the `reconcile` job already run an earlier, separate "Require rollout GitHub App configuration" step that checks the same two variables before any token is minted -- so the new steps are equivalent, not byte-identical, and are named accordingly ("Require Dev Platform GitHub App" in `rollout.yml`'s `rollout` job, "Require Dev Platform operator repository" in `reconcile-stale-rollouts.yml`'s `reconcile` job).
