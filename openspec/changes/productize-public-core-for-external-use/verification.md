...
- The first archive attempt exposed incomplete synthetic-fixture migration and a missing `scm_provider` render context; both were repaired with focused regression coverage. The final archive run re-runs the authoritative selected checks and writes the successful `automated-checks.json` receipt.
