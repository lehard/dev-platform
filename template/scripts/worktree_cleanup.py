...
from _platform_common import atomic_write_text, github_cli_env, machine_path, main_root, read_platform_config, run_git, scm_provider
...
def _exact_worktree_pr_is_merged(root: Path, config: dict, worktree: Worktree, main_branch: str) -> bool:
    """Whether GitHub authoritatively proves this worktree's branch was merged.

    Local ancestry cannot prove a squash- or rebase-merged branch is
    integrated: the rewritten commit on main is never an ancestor of the
    original branch tip. This reuses the same exact-head publication lookup
    the finish/reconcile lifecycle and the agent-board scope gate already
    trust, and fails closed (not merged) whenever GitHub evidence is
    unavailable, the worktree has no branch, or the exact head does not
    match a MERGED PR.
    """
    if not worktree.branch or scm_provider(config) != "github":
        return False
    env = github_cli_env(root)
    if env is None:
        return False
    lookup = publication_state.find_exact_head_pr(root, env, worktree.branch, main_branch, worktree.head)
