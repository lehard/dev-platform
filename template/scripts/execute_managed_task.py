...
def execute_managed_task(root, args: argparse.Namespace):
    ...
    # Authoring creates Backlog state, so the composed entrypoint must prove
    # admission before it reaches ``create_task``.  ``start_managed_task``
    # repeats the gate to keep its public entrypoint independently safe.
