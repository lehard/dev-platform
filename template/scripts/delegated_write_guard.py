...
# Bounded closed vocabulary for the final containment classification. "clean"
# is no observed head/path movement at all. "verified_external_advance" is a
# pure integration-head move proven to be a concurrent, legitimate lifecycle
# fast-forward to the checkout's own recorded remote main -- not a delegated
# escape. Anything else remains "violation". Routing/archive gates must key
# off this field rather than inferring safety from `returncode == 0` alone.
CLASSIFICATION_CLEAN = "clean"
CLASSIFICATION_VERIFIED_EXTERNAL_ADVANCE = "verified_external_advance"
CLASSIFICATION_VIOLATION = "violation"
...
class GuardedRunResult:
    ...
    # Bounded audit trail for the containment classification decision. `containment`
    # above is left untouched (the raw, conservative observation); these fields record
    # the actual heads compared and the classification the guard derived from them, so
    # a routing receipt can distinguish "clean", "violation" and
    # "verified_external_advance" without re-deriving anything from `returncode`.
    classification: str = CLASSIFICATION_CLEAN
...
def _classify_containment(
    containment: ContainmentResult, tier: EnforcementTier, integration_root: Path, before_head: str, after_head: str
) -> str:
    """Downgrade a pure integration-head move to a verified external advance, or not.

    `containment` (from `delegation_containment.check_containment`) stays the
    raw, conservative observer and is never modified by this function -- this
    is the one layer allowed to reclassify its `violated=True` result, because
    only here is the enforcement tier and base-branch topology known. Every
    condition below is required; any one failing keeps the result a violation.
    """
    if not containment.violated:
...
def run_observed_delegation(
    *,
    ...
    classification = _classify_containment(containment, tier_decision.tier, integration_root, before.head, after.head)
    violation = classification == CLASSIFICATION_VIOLATION
