...
For a narrow question that would otherwise require bulk exploratory reading of bounded repository files, an opted-in project may prefer the provider-local context worker before loading that bulk context into an R2/R3 session. This is not a task-route change: it resolves through the existing `routine` profile, returns compact path/range/findings/uncertainty evidence, and direct targeted reads remain available for editing, debugging, architecture reasoning, and exact verification.

The capability is soft and disabled by default in `[context_delegation]`; it never blocks or redirects normal repository reads. Prepare a JSON request with a `question` and a bounded `scope` list of repository-relative `{ "path", "start_line"?, "end_line"? }` objects, then run:
...
The integration-owned `.claude/model-routing/<change>.json` record is the durable routing/execution receipt. Managed archive readiness and terminal `finish` bind it to the exact `source_issue` and change from task-local/canonical provenance, resolving that provenance from either the active change or its matching archive entry. They never select another active or archived task opportunistically, so a missing active directory is not routing success after archive.

Routing preparation and dispatch remain active-change operations. After archive, `route-codex` and `route-claude` cannot repair missing history with a late route.

Complex work retained by the current strong parent, and routine/standard work retained only when the standalone parent-only topology prevents a safely contained child writer, must record a truthful final outcome after implementation:

```bash
python3 scripts/model_routing.py record-retained-execution --reason "complex R3 work completed by the current supervisor"
```
...
Every model/effort value carries a `source`: `selected` (platform-chosen, not independently confirmed), `runtime-confirmed` (the runtime itself returned it), `self-reported` (a claim the supervisor itself makes, with no platform-owned observer), or `unknown`. Free-form model self-identification is never authoritative -- verify what the current supported Codex/Claude Code runtime can actually confirm before claiming a value, and degrade to `unknown` rather than guess. A route that was only prepared, not actually executed, has no participant at all; fallback and escalation preserve the actual path that ran, not the preferred one.

Claude Code has no platform-verifiable launch receipt: the native Agent tool is invoked by the supervisor's own tool call and its result reaches only the supervisor's conversation. `record-claude-execution` therefore records the supplied id only as a self-reported claim (`execution.outcome: "claimed"`, `execution.launch_evidence: "self-reported"`, `execution.launched: null`) rather than `launched: true`, and attaches a `claimed_participant` (same shape as `participant`, model `source: "self-reported"`) instead of an executed one. The routing gate accepts this claim only on a verifiable clean containment postcheck, never as launch proof, and efficiency/calibration reports never count it as launched.
